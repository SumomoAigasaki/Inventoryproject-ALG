...
function validar_permisos($rol, $PermisoRLS)
{
  ...
  echo"Permiso de Rol: " . $PermisoRLS . "<br>";
  ...
  while ($row = $stmt->fetch_assoc()) {
    ...
    if ($lineNumber === $position) {
      echo "<td align='center'> 
              <a href='../views/update_permissions.php?p=" . $row['RLS_Description'] . "' class='btn btn-outline-primary btn-sm' title='Editar Permisos del Rol " . $idrol . " '>
    ...
    foreach ($countRoles as $countRole) {
      ...
      if ($row['USP_IDtbl_user_privileges'] == $countRole['id']) {
        echo "<pre>";
        echo "<p><b>1. valores de Cadena</b></p>";
        print_r($countRoles);
        echo "<p><b>2.Lista de los ID de los roles</b></p>";
        $ids = array_column($countRoles, 'id');
        print_r($ids);
        echo "<p></p>";
        echo "<p> <b>1.-</b> El primer valor es: " . $countRoles[0]['id'] . "</p>";
        // print_r($countRoles[0]['id']);
        echo "</pre>";
        ...
        $listIdRol = array_column($countRoles, 'id');
        $firstArrayValue = $listIdRol[0];
        foreach ($listIdRol as $index => $id) {
          if ($index === 0) {

            // Obtener el índice del primer valor en el array
            $previousCount = array_search($firstArrayValue, $ids);

            // Aplicar tu lógica condicional
            ...
            echo "Primer Valor: " . $firstArrayValue . "<br>";
            echo "#id rol: " . $id . "<br>";
            // Imprimir los valores de posición y borde
            echo "-----------------------------------<br>";
            echo "Indice del Primer valor: " . $previousCount . "<br>";
            echo "Posición: " . $position . "<br>";
            echo "Borde: " . $border . "<br>";
          }
        }
